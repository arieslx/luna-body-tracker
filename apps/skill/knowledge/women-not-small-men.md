---
id: women-not-small-men
title: Women are not small men
organization: Luna Body Tracker local knowledge draft
versionDate: 2026-06-15
tags: female-physiology, training, nutrition, recovery
---
### Planned / Not Started
- [ ] placeholder

Women-centered recommendations should not treat male physiology as the default and then scale down. Training tolerance, recovery needs, sleep, energy availability, cycle symptoms, stress, and iron status can all affect what feels sustainable.

For MVP recommendations, use this source as a framing principle rather than a rigid rule. Start from the user's own records, goals, and constraints. Prefer conservative adjustments that can be tested over a few days.
